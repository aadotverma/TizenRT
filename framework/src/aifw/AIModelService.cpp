...
{
	if (mTimer) {
		system_result res = system_timer_destroy(mTimer);
...
AIFW_RESULT AIModelService::setInterval(uint16_t interval)
{
	...
	ret = system_timer_change_interval(mTimer, (unsigned int)interval);
...
AIFW_RESULT AIModelService::prepare(void)
{
	...
	if (mInterval > 0) {
		mTimer = (system_timer *)calloc(1, sizeof(system_timer));
		if (mTimer == NULL) {
		...
		system_result ret;
		/* create timer */
		ret = system_timer_create(mTimer, (void *)timerTaskHandler, (void *)this, mInterval);
		if (ret != SYSTEM_SUCCESS) {
