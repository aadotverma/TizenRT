...
static bool checkId3V2Header(const unsigned char *buffer, size_t size, bool isFile, FILE *fp = nullptr)
{
	if (isFile && !fp) {
		meddbg("File pointer given null in file based format\n");
		return false;
	}
...
{
	bool ret = checkId3V2Header(buffer, size, false);
	if (!ret) {
...
bool file_header_parsing(FILE *fp, audio_type_t audioType, unsigned int *channel, unsigned int *sampleRate, audio_format_type_t *pcmFormat)
{
	...
	case AUDIO_TYPE_MP3:
		...
		ret = fread(id3Header, sizeof(unsigned char), MP3_ID3V2_HEADER_LENGTH, fp);
		res = checkId3V2Header(id3Header, ret, true, fp);
		if (!res) {
			meddbg("check for Id3V2 header fails\n");
			return false;
		}
